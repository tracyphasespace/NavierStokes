...
theorem beta_controls_both_c_and_hbar (vac1 vac2 : VacuumMedium) (sol : VortexSoliton)
  ...
  unfold angular_impulse sonic_velocity
  rw [h_same_rho]
  simp only [Real.sqrt_div (le_of_lt vac2.beta_pos), Real.sqrt_div (le_of_lt vac1.beta_pos)]
  have h_gamma_ne : sol.gamma_shape ≠ 0 := ne_of_gt sol.gamma_pos
  have h_mass_ne : sol.mass_eff ≠ 0 := ne_of_gt sol.mass_pos
  have h_radius_ne : sol.radius ≠ 0 := ne_of_gt sol.radius_pos
  have h_rho_ne : Real.sqrt vac2.rho ≠ 0 := ne_of_gt (Real.sqrt_pos_of_pos vac2.rho_pos)
  have h_beta1_ne : Real.sqrt vac1.beta ≠ 0 := ne_of_gt (Real.sqrt_pos_of_pos vac1.beta_pos)
  have h_beta2_ne : Real.sqrt vac2.beta ≠ 0 := ne_of_gt (Real.sqrt_pos_of_pos vac2.beta_pos)
  field_simp [h_gamma_ne, h_mass_ne, h_radius_ne, h_rho_ne, h_beta1_ne, h_beta2_ne]
  ring
