...
The unitary group establishes that the operator $\D^2$ has well-defined spectral theory and that its kernel---the constraint surface $\D^2 \Psi = 0$---is a closed subspace of $\mathcal{H}$.
\end{proof}
...
\end{theorem}
\begin{proof}
The transport equation $\partial_t \Psi + p \cdot \nabla_x \Psi = 0$ is solved by the method of characteristics: $\Psi(t, x, p) = \Psi_0(x - tp, p)$. The flow $(x,p) \mapsto (x - tp, p)$ is a volume-preserving diffeomorphism of $\R^3 \times \T^3$, so:
\begin{equation}
...
\textbf{Step 2: Evolve.} By the transport equation (Section \ref{sec:wellposed}), there exists a unique solution $\Psi(t)$ with $\Psi(0) = \Psi_0$, preserving the scleronomic constraint.
...
\textbf{Step 5: Regularity.} By Theorem \ref{thm:regularity} (Sobolev regularity preservation), $\|\Psi(t)\|_{H^k} = \|\Psi_0\|_{H^k}$ for all $k \geq 0$. Since projection against the smooth weight $\rho \in C^\infty(\T^3)$ preserves Sobolev order:
\begin{equation}
    \|u(t)\|_{H^k(\R^3)} \leq C_k \|\Psi(t)\|_{H^k} = C_k \|\Psi_0\|_{H^k} < \infty
\end{equation}
for all $k$ and all $t \geq 0$. By the Sobolev embedding $H^k(\R^3) \hookrightarrow C^{k-2}(\R^3)$ for $k > 5/2$, the solution $u(t)$ is smooth for all time. Blow-up---characterized by $\|\nabla^s u(t)\|_{L^2} \to \infty$ for some Sobolev index $s$---cannot occur because \textit{all} Sobolev norms are uniformly bounded. \\
