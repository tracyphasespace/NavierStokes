...
\textit{Lean 4:} \texttt{Phase7\_Density/FunctionSpaces.lift\_preserves\_regularity}
\end{proof}
...
The lift is not unique---any smooth weight function $\rho$ satisfying the normalization condition produces a valid lift. This non-uniqueness reflects the physical reality that many microscopic configurations can produce the same macroscopic velocity field.

\begin{proposition}[Weight Function Freedom]
If $\rho_1$ and $\rho_2$ are both smooth weight functions, then:
\begin{equation}
    \Proj(\Lambda_{\rho_1}(u)) = \Proj(\Lambda_{\rho_2}(u)) = u
\end{equation}
...
Different choices of $\rho(p)$ correspond to different microscopic momentum distributions:

\begin{itemize}
    \item \textbf{Uniform $\rho$:} Equal momentum in all directions (isotropic)
    \item \textbf{Gaussian $\rho$:} Maxwell-Boltzmann thermal equilibrium
    \item \textbf{Anisotropic $\rho$:} Directional momentum bias (e.g., shear flows)
\end{itemize}

The projection ``averages out'' these microscopic details, recovering the bulk velocity $u(x)$. This is precisely what Navier and Stokes did experimentally---they measured the projected dynamics without resolving the momentum distribution.
...
Regularity Preservation & \texttt{FunctionSpaces.lift\_preserves\_regularity} & \checkmark \\
